...
export const getAsset = (userId, assetId) => (dispatch) => {
  return APIUtil.fetchAsset(userId, assetId).then(
    (asset) => dispatch(receiveAsset(asset)),
    (err) => dispatch(receiveAssetErrors(err.responseJSON))
  );
};
...
export const getAllAssets = (userId) => (dispatch) => {
  return APIUtil.fetchAssets(userId).then(
...
export const buyAsset = (userId, asset) => (dispatch) => {
  return APIUtil.createAsset(userId, asset).then(
    (asset) => dispatch(receiveAsset(asset)),
    (err) => dispatch(receiveAssetErrors(err.responseJSON))
  );
};
...
export const updateOldAsset = (userId, asset) => (dispatch) => {
  return APIUtil.updateAsset(userId, asset).then(
    (asset) => dispatch(receiveAsset(asset)),
    (err) => dispatch(receiveAssetErrors(err.responseJSON))
  );
};
...
export const sellAsset = (userId, assetId) => (dispatch) => {
  return APIUtil.destroyAsset(userId, assetId).then(
    (asset) => dispatch(deleteAsset(asset)),
    (err) => dispatch(receiveAssetErrors(err.responseJSON))
  );
};
